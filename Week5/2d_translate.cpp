...
void drawLine(int x1,int y1,int x2,int y2)
{
    ...
    if(dx >= dy) {
        ...
        while(x1 <= x2)
        {
            ...
            glVertex2i(x1 + x_shift, y1 + y_shift);
        }
...
{ 
  glBegin(GL_POINTS);
    ...
    glVertex2i(xc+x+ x_shift, yc+y+ y_shift); 
    glVertex2i(xc-x+ x_shift, yc+y+ y_shift); 
    glVertex2i(xc+x+ x_shift, yc-y+ y_shift); 
    glVertex2i(xc-x+ x_shift, yc-y+ y_shift); 
    glVertex2i(xc+y+ x_shift, yc+x+ y_shift); 
    glVertex2i(xc-y+ x_shift, yc+x+ y_shift); 
    glVertex2i(xc+y+ x_shift, yc-x+ y_shift); 
    glVertex2i(xc-y+ x_shift, yc-x+ y_shift); 
...
void drawPolygon(int sides)
{
  ...
  glBegin(GL_POLYGON);
    ...
    for(int i = 0; i < sides; i++)
    {
      ...
      glVertex2i(100*cos(angle) + x_shift, 100*sin(angle) + y_shift);
    }
...
int main(int argc, char** argv) {
  ...
  switch(choice)
  {
    case 1: {
      ...
      printf("Enter how much the x axis and y axis have to be shifted by\n");
